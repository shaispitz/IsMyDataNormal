...
import { Upload, TestTube, AlertCircle, Loader2, Printer, Play } from 'lucide-react';
...
// Robust parser for numerical data
...
export default function NormalityAnalysis() {
        ...
        script.onload = () => {
          ...
          if (loadedCount === scriptUrls.length) {
            if (checkScripts()) {
                 ...
                 reject(new Error("A script was loaded but its global object is not available."));
            }
          }
        };
        script.onerror = (err) => {
  ...
  const runAnalysis = async (dataText) => {
    if (Object.values(selectedTests).every(v => !v)) {
    ...
    try {
      ...
      if (data.length < 8) {
        throw new Error(`At least 8 valid numerical data points are required for the tests. Found: ${data.length}`);
      }
  ...
  const handleAnalyze = async () => {
    if (!file) {
      setError("Please select a file to run the analysis.");
      return;
    }
        ...
        <div className="no-print">
            <header className="text-center mb-8">
              ...
              <p className="mt-2 text-lg text-slate-600">
                Upload your data file, or use our sample data, and select tests to check for normal distribution.
              </p>
            </header>
            ...
            <Card className="mb-8 shadow-sm">
              <CardContent className="p-6">
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-3">2. Get Data</h3>
                    <div className="flex items-center gap-4">
                     ...
                     <p className="text-xs text-slate-500 mt-2 mb-4">CSV or text file with numbers, max 50MB.</p>
                     <Button variant="secondary" onClick={handleSampleData} disabled={isAnalyzing || !scriptsReady}>
                ...
                <div className="mt-6 text-center">
                  <Button size="lg" onClick={handleAnalyze} disabled={(!file && fileName !== 'Sample Data') || isAnalyzing || !scriptsReady}>
                    {isAnalyzing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Analyzing...</> : <><TestTube className="mr-2 h-4 w-4" />Analyze Data</>}
                  </Button>
                </div>
              </CardContent>
            </Card>
