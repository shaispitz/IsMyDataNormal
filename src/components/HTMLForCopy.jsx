...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            main { padding: 0 !important; max-width: 100% !important; margin: 0 auto !important; }
            .shadow-md, .shadow-sm, .shadow { box-shadow: none !important; border: 1px solid #e2e8f0 !important; }
            .space-y-6 > *, .mb-6, .mb-8 { page-break-inside: avoid; }
            canvas { display: block !important; width: 100% !important; height: auto !important; }
        }
        .lucide-icon { width: 1rem; height: 1rem; display: inline-block; vertical-align: middle; }
    </style>
</head>
<body class="bg-slate-50">
    ...
    <script type="text/babel">
        ...
        const AlertCircle = () => React.createElement('svg', { className: 'lucide-icon', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2' }, React.createElement('circle', { cx: '12', cy: '12', r: '10' }), React.createElement('line', { x1: '12', y1: '8', x2: '12', y2: '12' }), React.createElement('line', { x1: '12', y1: '16', x2: '12.01', y2: '16' }));
        const Loader2 = () => React.createElement('svg', { className: 'lucide-icon animate-spin mr-2', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2' }, React.createElement('path', { d: 'M21 12a9 9 0 11-6.219-8.56' }));
        const Printer = () => React.createElement('svg', { className: 'lucide-icon mr-2', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2' }, React.createElement('polyline', { points: '6,9 6,2 18,2 18,9' }), React.createElement('path', { d: 'M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2' }), React.createElement('rect', { x: '6', y: '14', width: '12', height: '8' }));
        ...
        const Card = ({ children, className = "" }) => React.createElement('div', { className: ['bg-white', 'rounded-lg', 'shadow-sm', 'border', 'border-gray-200', className].join(' ') }, children);
        const CardHeader = ({ children }) => React.createElement('div', { className: 'px-6 py-4 border-b border-gray-200' }, children);
        const CardTitle = ({ children, className = "" }) => React.createElement('h3', { className: ['text-lg', 'font-semibold', 'text-gray-900', className].join(' ') }, children);
        const CardContent = ({ children }) => React.createElement('div', { className: 'px-6 py-4' }, children);
        const Checkbox = ({ id, checked, onCheckedChange }) => React.createElement('input', { type: 'checkbox', id, checked, onChange: (e) => onCheckedChange(e.target.checked), className: 'h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded' });
        const Alert = ({ children, variant = "default", className = "" }) => {
            ...
            return React.createElement('div', { className: ['border', 'rounded-lg', 'p-4', 'flex', 'items-start', 'gap-3', variantClass, className].join(' ') }, children);
        };
        const AlertTitle = ({ children }) => React.createElement('h5', { className: 'font-medium' }, children);
        const AlertDescription = ({ children }) => React.createElement('div', { className: 'text-sm' }, children);
        ...
        const ChartDisplay = ({ data, type }) => {
            ...
            useEffect(() => {
                ...
                if (type === 'qq') {
                    ...
                    config = {
                        type: 'scatter',
                        data: { datasets: [ { label: 'Q-Q Data', data: data, backgroundColor: 'rgba(59, 130, 246, 0.7)', }, { label: 'Reference Line', data: [{x: min, y: min}, {x: max, y: max}], type: 'line', borderColor: 'rgba(239, 68, 68, 0.8)', fill: false, pointRadius: 0, borderWidth: 2, } ] },
                        options: { responsive: true, maintainAspectRatio: false, scales: { x: { title: { display: true, text: 'Theoretical Quantiles (Standard Normal)' } }, y: { title: { display: true, text: 'Sample Quantiles (Standardized)' } } }, plugins: { tooltip: { callbacks: { label: function(c) { return 'Theoretical: ' + c.raw.x.toFixed(3) + ', Sample: ' + c.raw.y.toFixed(3); } } } } },
                    };
                } else if (type === 'hist_normal') {
                    config = {
                        data: { datasets: [ { label: 'Histogram', data: data.histogram, backgroundColor: 'rgba(59, 130, 246, 0.6)', type: 'bar', barPercentage: 1.0, categoryPercentage: 1.0, }, { label: 'Normal Distribution', data: data.normalCurve, borderColor: 'rgba(239, 68, 68, 0.8)', type: 'line', pointRadius: 0, borderWidth: 3, fill: false, } ] },
                        options: { responsive: true, maintainAspectRatio: false, scales: { x: { type: 'linear', title: { display: true, text: 'Data Value' }, }, y: { title: { display: true, text: 'Frequency' }, beginAtZero: true }, }, plugins: { tooltip: { mode: 'index', intersect: false, } } },
                    };
                }
                if (chartRef.current) { chartRef.current.destroy(); }
                chartRef.current = new window.Chart(canvasRef.current.getContext('2d'), config);
            }, [data, type]);
            return React.createElement('div', { className: 'relative h-full w-full' }, React.createElement('canvas', { ref: canvasRef }));
        };
            ...
            return React.createElement(Card, { className: 'shadow-md transition-shadow hover:shadow-lg' },
                React.createElement(CardHeader, {},
                    React.createElement(CardTitle, { className: 'flex items-center gap-3 text-xl text-slate-800' },
                        React.createElement('div', { className: 'flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center' }, React.createElement(TestTube, { className: 'w-5 h-5 text-blue-600' })),
                        title
                    )
                ),
        ...
        const HorizontalGauge = ({ value }) => {
            ...
            const ranges = [ { limit: 0.1, color: '#22c55e', label: 'Very Close' }, { limit: 0.5, color: '#3b82f6', label: 'Reasonable' }, { limit: 1.0, color: '#ef4444', label: 'Noticeable' }, { limit: GAUGE_MAX, color: '#1f2937', label: 'Strong Deviation' }, ];
            return React.createElement('div', { className: 'flex flex-col items-center space-y-6 w-full max-w-2xl mx-auto' },
                React.createElement('div', { className: 'w-full relative' },
                    React.createElement('div', { className: 'h-6 bg-gray-200 rounded-full relative overflow-hidden' }, ...ranges.map((range, index) => { const startPercent = index === 0 ? 0 : (ranges[index - 1].limit / GAUGE_MAX) * 100; const endPercent = (range.limit / GAUGE_MAX) * 100; const width = endPercent - startPercent; return React.createElement('div', { key: index, className: 'absolute top-0 h-full', style: { left: startPercent + '%', width: width + '%', backgroundColor: range.color } }); })),
                    React.createElement('div', { className: 'absolute top-0 transform -translate-x-1/2 transition-all duration-500 ease-out', style: { left: percentage + '%' } }, React.createElement('div', { className: 'w-1 h-6 bg-gray-800' }), React.createElement('div', { className: 'absolute -top-2 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-b-4 border-transparent border-b-gray-800' }))
                ),
                React.createElement('div', { className: 'grid grid-cols-4 gap-2 w-full text-center' }, ...ranges.map(range => React.createElement('div', { key: range.label }, React.createElement('p', { className: 'text-xs font-semibold', style: { color: range.color } }, range.label))))
            );
        };

        const KlDivergenceDisplay = ({ result }) => {
            const { score, mean, stdDev, dataSize } = result;
            return React.createElement(TestResultCard, { title: 'KL Divergence' }, React.createElement('div', { className: 'grid md:grid-cols-3 gap-6' }, React.createElement('div', { className: 'md:col-span-2 space-y-4' }, React.createElement('p', { className: 'text-2xl font-bold text-slate-800' }, score.toFixed(4)), React.createElement(HorizontalGauge, { value: score })), React.createElement('div', { className: 'bg-slate-50 p-4 rounded-lg space-y-2 text-sm' }, React.createElement('p', {}, React.createElement('strong', {}, 'Mean: '), mean.toFixed(3)), React.createElement('p', {}, React.createElement('strong', {}, 'Std Dev: '), stdDev.toFixed(3)), React.createElement('p', {}, React.createElement('strong', {}, 'Data Size: '), dataSize))));
        };

        const AndersonDarlingDisplay = ({ result }) => {
            const { statistic } = result;
            const criticalValues = { '15%': 0.576, '10%': 0.656, '5%': 0.787, '2.5%': 0.918, '1%': 1.092, };
            ...
            return React.createElement(TestResultCard, { title: 'Anderson-Darling Test' },
                React.createElement('div', { className: 'grid md:grid-cols-2 gap-6 items-start' },
                    React.createElement('div', {},
                        ...
                        React.createElement('div', { className: 'mt-4 flex items-center gap-2 font-semibold ' + (pass ? 'text-green-600' : 'text-red-600') }, pass ? React.createElement(CheckCircle2, { className: 'w-5 h-5' }) : React.createElement(XCircle, { className: 'w-5 h-5' }), React.createElement('span', {}, 'At ' + significanceLevel + ' significance, the data ' + (pass ? 'passes' : 'fails') + ' the normality test.')),
                        React.createElement('p', { className: 'text-xs text-slate-500 mt-1' }, '(The null hypothesis of normality cannot be rejected if the statistic is less than the critical value).')
                    ),
                    React.createElement('div', { className: 'bg-slate-50 p-4 rounded-lg' },
                        ...
                        React.createElement('ul', { className: 'space-y-1 text-sm' }, ...Object.entries(criticalValues).map(([level, value]) => React.createElement('li', { key: level, className: 'flex justify-between' }, React.createElement('span', { className: 'text-slate-600' }, 'Significance ' + level + ':'), React.createElement('span', { className: 'font-mono ' + (statistic > value ? 'text-red-500 font-bold' : 'text-slate-800') }, value))))
                    )
                )
            );
        };
        ...
        const GeneralStatsDisplay = ({ stats }) => {
            ...
            const statItems = [ { label: 'Data Size', value: dataSize.toLocaleString() }, { label: 'Mean', value: mean.toFixed(3) }, { label: 'Median', value: median.toFixed(3) }, { label: 'Std Dev', value: stdDev.toFixed(3) }, { label: 'Min', value: min.toFixed(3) }, { label: 'Max', value: max.toFixed(3) }, ];
            return React.createElement(Card, { className: 'shadow-md mb-6 border-blue-200 border-2' },
                React.createElement(CardHeader, {},
                    React.createElement(CardTitle, { className: 'flex items-center gap-3 text-xl text-slate-800' },
                        React.createElement('div', { className: 'flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center' }, React.createElement(ListChecks, { className: 'w-5 h-5 text-blue-600' })),
                        'Data Summary'
                    )
                ),
                React.createElement(CardContent, {},
                    React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-6 gap-4 mb-6 text-center' }, ...statItems.map(item => React.createElement('div', { key: item.label, className: 'bg-slate-50 p-3 rounded-lg' }, React.createElement('p', { className: 'text-xs text-slate-600 mb-1' }, item.label), React.createElement('p', { className: 'text-lg font-bold text-slate-800' }, item.value)))),
                    distributionData && React.createElement('div', { className: 'w-full mt-4' }, React.createElement('h4', { className: 'text-center font-semibold text-slate-700 mb-2' }, 'Data Distribution vs. Normal'), React.createElement('div', { className: 'relative h-96' }, React.createElement(ChartDisplay, { data: distributionData, type: 'hist_normal' })))
                )
            );
        };
        ...
        // Main App Component
        const App = () => {
            const [selectedTests, setSelectedTests] = useState({ kl: true, ad: true, qq: true });
            ...
            const handleFileChange = (e) => {
                ...
                if (uploadedFile) {
                    if (uploadedFile.size > 50 * 1024 * 1024) { setError("File size cannot exceed 50MB."); return; }
                    ...
                    React.createElement('div', { className: 'no-print' },
                        React.createElement('header', { className: 'text-center mb-8' },
                            React.createElement('div', { className: 'flex justify-center items-center gap-4' },
                                ...
                                React.createElement(Button, { variant: 'outline', onClick: handleExport, disabled: !results }, React.createElement(Printer), 'Export PDF')
                            ),
                            React.createElement('p', { className: 'mt-2 text-lg text-slate-600' }, 'Upload your data file and select tests to check for normal distribution.')
                        ),
                            ...
                            React.createElement(CardContent, { className: 'p-6' },
                                React.createElement('div', { className: 'grid md:grid-cols-2 gap-8' },
                                    React.createElement('div', {},
                                        ...
                                        React.createElement('div', { className: 'grid grid-cols-1 gap-3' }, ...Object.entries(TESTS).map(([key, name]) => React.createElement('div', { key, className: 'flex items-center space-x-2' }, React.createElement(Checkbox, { id: key, checked: !!selectedTests[key], onCheckedChange: (checked) => setSelectedTests(prev => ({ ...prev, [key]: checked })) }), React.createElement('label', { htmlFor: key, className: 'text-sm font-medium text-slate-700 cursor-pointer' }, name))))
                                    ),
                                    React.createElement('div', {},
                                        React.createElement('h3', { className: 'text-lg font-semibold mb-3' }, '2. Upload File'),
                                        React.createElement('div', { className: 'flex items-center gap-4' },
                                            React.createElement(Button, { variant: 'outline', onClick: () => fileInputRef.current?.click() }, React.createElement(Upload), 'Choose File'),
                                            React.createElement('input', { ref: fileInputRef, type: 'file', accept: '.csv, .txt, text/plain', onChange: handleFileChange, className: 'hidden' }),
                        ...
                        isAnalyzing && React.createElement('div', { className: 'text-center p-8' }, React.createElement(Loader2, { className: 'h-8 w-8 mx-auto animate-spin text-blue-600 mb-4' }), React.createElement('p', { className: 'text-slate-600' }, 'Running analyses... this may take a moment.'))
                    ),
                    generalStats && React.createElement(GeneralStatsDisplay, { stats: generalStats }),
                    results && React.createElement('div', { className: 'space-y-6' }, ...Object.keys(results).map(key => { if (key === 'kl') return React.createElement(KlDivergenceDisplay, { key, result: results.kl }); if (key === 'ad') return React.createElement(AndersonDarlingDisplay, { key, result: results.ad }); if (key === 'qq') return React.createElement(TestResultCard, { key, title: 'Q-Q Plot' }, React.createElement('div', { className: 'relative h-80' }, React.createElement(ChartDisplay, { data: results.qq, type: 'qq' }))); return null; }))
                )
            );
        };
